const returnToDashboard = () => {
	...
	document.getElementById('confirm_message').textContent = "Return to dashboard without saving?"
...
const openEditFullName = (editButton) => {
	...
	if(saveFullName.classList.contains('hidden') && cancelFullName.classList.contains('hidden')){
...
const handleSaveFullName = async(saveFullName) => {
	...
	const fullNameProfile = (fullNameInput.value) ? fullNameInput.value.trim() : fullNameInput.placeholder.trim();

	// if(!validateFullNameInput(fullNameProfile)) return;

	console.log(validateFullNameInput());
